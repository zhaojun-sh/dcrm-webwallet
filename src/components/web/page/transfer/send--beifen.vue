          ...
          <table class="table table-bordered table-hover">
            ...
            <tbody>
              <tr v-for="item in historyData" :key="item.index">
                <td>
                  <span v-html="item.status" :class="item.status=='failure'?'red':''"></span>
                </td>
                <td>
                ...
                <td>
                  <span v-html="item.info" :title="item.info" class="ellipsis"></span>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  mounted() {
    ...
    this.publicKey = this.$store.state.publicKey;
    // this.walletAdress = this.$store.state.addressInfo
    this.walletAdress = "3a1b3b81ed061581558a81f11d63e03129347437";
    // console.log(Web3)
    // console.log(Web3.currentProvider)
    // let Web3 = new Web3(Web3.givenProvider || this.$$.baseUrl)
    ...
    // this.web3Set = Web3
    // this.setWeb3()
    // this.dcrmSignTranForEth()
    // console.log(this.web3Set)
    this.historyData = [
      {
        status: "failure",
        coin: "FSN",
        amount: this.$$.thousandBit(500, 0),
        date: "2018/10/22/15:46",
        info: "0xfd02759fba7aa4ddb1a9e361d7ea6fa1e..."
      }
    ];
    // this.selectData = ''
    this.sendAmound = this.$$.thousandBit("20000");
  ...
  methods: {
    ...
    changeAmount() {
      this.sendAmound = this.$$.thousandChange(this.sendAmound);
    },
    setWeb3() {
      let Web3 = require("web3");
      if (typeof web3 !== "undefined") {
    ...
    sendAmoundInfo() {
      this.setWeb3();
      // console.log(this.web3Set.isConnected())
      let rawTx = {
        nonce: "0",
        gasPrice: "400",
        gasLimit: "300",
      ...
      // this.web3Set.eth.getAccounts( (err, res) => {
      //   console.log(err)
      //   console.log(res)
      // })
      // let blockTest = this.web3Set.eth.getBlock(196611)
      // console.log(blockTest)
      // return
      // console.log(this.web3Set.isAddress(rawTx.from))
      // console.log(this.web3Set.isAddress(rawTx.to))
      // this.web3Set.eth.sendTransaction(rawTx, (err, res) => {
      //   if (err) {
      //     console.log(err)
      //   } else {
      //     console.log(res)
      //   }
      // })
      // let transaction = this.web3Set.eth.getTransaction('0xd562bb03a071f8928f885946be4bd37b5beaed6e', (err, res) => {
      //   console.log(err)
      //   console.log(res)
      // })
      ...
      let privateKey = new Buffer(
        "e63d00f899b00b98b9f56d260d677af0b7ae5564feab9444afc808c914b19e3d",
        "hex"
      );
      ...
      this.web3Set.eth.sendRawTransaction(serializedTxString, (err, hash) => {
        if (!err) {
    ...
    dcrmSignTranForEth() {
      ...
      // let to_value = this.sendAmound
      let to_value = "0.01";
      let address = this.walletAdress;
      // let coin = this.selectData
      let coin = "ETH";
      console.log(this.web3Set.eth);
      if (to_address && to_value && address && coin) {
        to_address =
          to_address.toLowerCase().indexOf("0x") == 0
            ? to_address
            : "0x" + to_address;
        address =
          address.toLowerCase().indexOf("0x") == 0 ? address : "0x" + address;
        ...
        if (to_address.length == 42 && address.length == 42) {
          ...
          if (reg.test(to_value)) {
            ...
            // }
            // this.web3Set.setProvider(this.$$.baseUrl)
            // this.web3Set.setProvider(new this.web3Set.providers.WebsocketProvider('ws://localhost:8546'));
            // console.log(this.web3Set.eth)
            // console.log(this.web3Set.providers)
            // console.log(this.web3Set.currentProvider)
            this.web3Set.eth.getTransactionCount(address, (error, result) => {
              if (!error) {
                console.log("3" + result);

                let nonceHex = this.decToHex(result.toString());
                console.log("4" + nonceHex);
                if (coin == "ETH") {
                  let value = this.web3Set.toWei(to_value, "ether");
                  let valHex = this.decToHex(value);
                  this.getEthTxHash(to_address, valHex, nonceHex);
                  ...
                  let MMTContract = this.web3Set.eth
                    .contract(contractABI)
                    .at(contractAddress);
                  //let decimal = MMTContract.decimals();
                  MMTContract.decimals.call((err, result) => {
                    let decimal = result.toString();
                    let coinval = parseFloat(to_value) * Math.pow(10, decimal);
                    console.log(coin, coinval);
                    let valHex = this.decToHex(coinval.toString());

                    let erc20_data = MMTContract.transfer.getData(
                      to_address,
                      valHex
                    );
    ...
    sendTransaction() {
      ...
      if (tx_hash) {
        ...
        if (coin == "BTC") {
          ...
          //ETH ERC20 Broadcast Transaction
          this.web3Set.eth.sendRawTransaction(tx_hash, (err, hash) => {
            // hideMaskLayer();
            if (err) {
              console.log(err);
    ...
    decToHex(decStr) {
      ...
      let hex = this.convertBase(decStr, 10, 16);
