          ...
          <table class="table table-bordered table-hover">
            ...
            <tbody>
              <tr v-for="item in historyData" :key="item.index">
                <td>
                  <span v-html="item.statusFsn" :class="item.statusFsn=='failure'?'red':''"></span>
                </td>
                <td>
...
export default {
  ...
  methods: {
    ...
    setBaseSendData() {
      ...
      if (this.selectData.coin === "FSN") {
        try {
          this.balanceNum = this.web3.fromWei(
            this.web3.eth.getBalance(this.$store.state.addressInfo),
            "ether"
          );
        } catch (error) {
      ...
      // this.balanceNum = this.web3.fromWei(this.web3.eth.getBalance(this.walletAddress), 'ether')
    ...
    privateSure() {
      ...
      // console.log(this.newWeb3)
      this.newWeb3.lilo
        .dcrmGetAddr(this.walletAddress, this.selectData.coin)
        .then(val => {
          // console.log(val)
          this.dataPage = {
            nonce: this.nonceNum,
    ...
    getDatabaseInfo() {
      this.setWeb3();
      $.ajax({
        url: this.$$.serverURL + "/lilo/lockOutHistory",
        type: "post",
        datatype: "json",
        data: { from: this.coinAddress, coin: this.selectData.coin },
        success: res => {
          // console.log(res)
          this.historyData = [];
          if (res.msg === "success") {
            for (let i = 0; i < res.info.length; i++) {
              ...
              res.info[i].value = this.web3.fromWei(res.info[i].value, "ether");
