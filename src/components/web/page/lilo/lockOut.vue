...
export default {
  ...
  methods: {
    ...
    getInitData () {
      ...
      let web3Balance
      try {
        web3Balance = v_web3.lilo.dcrmGetBalance(this.$store.state.addressInfo, this.selectData.coin)
      } catch (error) {
        web3Balance = 0
      }
      this.balanceNum = v_web3.fromWei(web3Balance, "ether")
    ...
    getDatabaseInfo () {
      $ajax.post(this.$$.serverURL + "/lilo/lockOutHistory", Qs.stringify({from: this.coinAddress, coin: this.selectData.coin})
      ).then(res => {
        res = res.data
        this.historyData = []
        if (res.msg === "success") {
