...
export default {
  ...
  methods: {
    MnemonicBtn () {
      ...
      // console.log(this.$store.state.HDpathInit)
      let wallets = new wallet(hdks.derive(this.$store.state.HDpath)._privateKey)
    ...
    inputTREZORBtn () {
      if (!this.$store.state.trezor) {
        this.$$.errTip(this.LANG.ERROR_TIP.TIP_3)
        return
      }
      this.checkAddress = this.$store.state.addressInfo
      this.sendType('TREZOR')
    },
    trezorStx (rawTx, stxData) {
      // let params = this.$$.bipPath+ "/" + 0
      let params = this.$store.state.HDpath
      // let data = rawTx.data
      console.log(rawTx)
      // rawTx.nonce = rawTx.nonce === 0 ? 1 : rawTx.nonce
      ...
      console.log(rawTx)
      let TrezorConnect = this.$store.state.trezor
    ...
    ledgerStx (txData, stxData) {
      var app = this.$store.state.ledger
      txData.path = this.$store.state.HDpath
    ...
    getDcrmAddress (type) {
      this.dcrmAddress = v_web3.lilo.dcrmGetAddr(this.$store.state.addressInfo, this.sendDataPage.coin)
      this.signSendData(type)
    },
    getDecrAddress (pwd, type) {
      ...
      try {
        ...
        get_dcrmReqAddr = error
      }

      let dcrmAddressVal
      if ((typeof get_dcrmReqAddr).toLowerCase() === "object") {
        if (get_dcrmReqAddr.result && get_dcrmReqAddr.result.indexOf("the account has request dcrm address already.the dcrm address is") === 0) {
          dcrmAddressVal = get_dcrmReqAddr.result.split(":")[1]
          ...
          sendBack.nowFlag = false
        } else if (get_dcrmReqAddr.result && get_dcrmReqAddr.result.indexOf("the account has confirm dcrm address already.the dcrm address is") === 0) {
          this.$$.successTip(this.LANG.ERROR_TIP.TIP_8 + get_dcrmReqAddr.result.split(":")[1])
          this.elDialogView()
          return
        } else {
